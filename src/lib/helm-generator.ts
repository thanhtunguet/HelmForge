...
type RancherQuestionType = 'string' | 'boolean' | 'password' | 'multiline';

interface RancherQuestion {
  variable: string;
  label: string;
  description?: string;
  type?: RancherQuestionType;
  required?: boolean;
  default?: string | number | boolean;
  group?: string;
}

export function generateQuestionsYaml(
  template: TemplateWithRelations,
  version: ChartVersion
): string {
  const questions: RancherQuestion[] = [];
  const nginxEnabled = version.values.enableNginxGateway ?? template.enableNginxGateway;
  const redisEnabled = version.values.enableRedis ?? template.enableRedis;

  questions.push({
    variable: 'global.registry.password',
  ...
  questions.push({
    ...
    label: 'Enable Nginx Gateway',
    type: 'boolean',
    default: nginxEnabled,
  ...
  questions.push({
    ...
    label: 'Enable Redis',
    type: 'boolean',
    default: redisEnabled,
    group: 'Optional Components',
  });

  template.services.forEach((service) => {
    const imageTagDefault = version.values.imageTags[service.name] || 'latest';
    questions.push({
      variable: `services.${service.name}.imageTag`,
      label: `Image Tag (${service.name})`,
      description: `Container image tag for ${service.name}.`,
      type: 'string',
      default: imageTagDefault,
      group: 'Services',
    });

    const serviceEnvValues = version.values.envValues[service.name] || {};
    service.envVars.forEach((envVar) => {
      const envDefault = serviceEnvValues[envVar.name] ?? envVar.defaultValue;
      const envQuestion: RancherQuestion = {
        variable: `services.${service.name}.env.${envVar.name}`,
        label: `${service.name} ${envVar.name}`,
        description: envVar.description,
        type: 'string',
        group: `${service.name} Environment`,
      };

      if (envVar.required !== undefined) {
        envQuestion.required = envVar.required;
      }

      if (envDefault !== undefined) {
        envQuestion.default = envDefault;
      }

      questions.push(envQuestion);
    });
  });

  template.configMaps.forEach((configMap) => {
    const configMapValues = version.values.configMapValues[configMap.name] || {};
    configMap.keys.forEach((key) => {
      const keyDefault = configMapValues[key.name] ?? key.defaultValue;
      const configQuestion: RancherQuestion = {
        variable: `configMaps.${configMap.name}.${key.name}`,
        label: `${configMap.name} ${key.name}`,
        description: key.description,
        type: 'string',
        group: `ConfigMap: ${configMap.name}`,
      };

      if (keyDefault !== undefined) {
        configQuestion.default = keyDefault;
      }

      questions.push(configQuestion);
    });
  });

  return formatYaml({ questions });
}

export function generateAppReadme(template: TemplateWithRelations, version: ChartVersion): string {
  const releaseNotes = version.releaseNotes?.trim() || 'No release notes provided.';
  return `# ${template.name} Release Notes\n\n${releaseNotes}\n`;
}

export function generateChartReadme(template: TemplateWithRelations): string {
  const templateReadme = template.readme?.trim();
  const fallbackDescription = template.description?.trim() || 'Helm chart generated by HelmForge.';
  return `# ${template.name}\n\n${templateReadme || fallbackDescription}\n`;
}
...
export async function downloadChart(
  ...
  // Helm chart README
  files.push({ name: `${chartName}/README.md`, content: generateChartReadme(template) });
