...
function generateQuestionsYaml(template: TemplateWithRelations, version: ChartVersion): string {
  const questions: RancherQuestion[] = [];
  const nginxEnabled = version.values.enableNginxGateway ?? template.enable_nginx_gateway;
  const redisEnabled = version.values.enableRedis ?? template.enable_redis;
  ...
  template.services.forEach((svc) => {
    const imageTagDefault = version.values.imageTags?.[svc.name] || 'latest';
    questions.push({
      variable: `services.${svc.name}.imageTag`,
      label: `Image Tag (${svc.name})`,
      description: `Container image tag for ${svc.name}.`,
      type: 'string',
      default: imageTagDefault,
      group: 'Services',
    });

    const serviceEnvValues = version.values.envValues?.[svc.name] || {};
    Object.entries(serviceEnvValues).forEach(([key, value]) => {
      questions.push({
        variable: `services.${svc.name}.env.${key}`,
        label: `${svc.name} ${key}`,
        type: 'string',
        default: value,
        group: `${svc.name} Environment`,
      });
    });
  });
  ...
  template.config_maps.forEach((cm) => {
    ...
    Object.entries(configMapValues).forEach(([key, value]) => {
      questions.push({
        variable: `configMaps.${cm.name}.${key}`,
        label: `${cm.name} ${key}`,
        type: 'string',
        default: value,
        group: `ConfigMap: ${cm.name}`,
      });
    });
  });

  return formatYaml({ questions } as Record<string, unknown>);
}

function generateAppReadme(template: TemplateWithRelations, version: ChartVersion): string {
  const releaseNotes = version.release_notes?.trim() || 'No release notes provided.';
  return `# ${template.name} Release Notes\n\n${releaseNotes}\n`;
}

function generateChartReadme(template: TemplateWithRelations): string {
...
async function generateChartPackage(template: TemplateWithRelations, version: ChartVersion): Promise<string> {
  ...
  // Helm chart README
  addFile(`${chartName}/README.md`, generateChartReadme(template));
